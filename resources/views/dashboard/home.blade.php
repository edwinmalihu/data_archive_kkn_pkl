  ...
  @section('container')
        ...
        <header class="panel-heading">
            <!-- <div class="export-fitur"> -->
              <h3 class="panel-title">Daftar Laporan Praktek Kerja Lapangan (PKL)</h3>
              <!-- <a href="/dashboard/exportPdf" class="btn btn-primary pdf">Report PDF PKL</a> -->
              ...
              @foreach( $pkl as $p )
                <tr class="gradeA">
                  ...
                  <td class="actions">
                    ...
                    <a href="/dashboard/{{$p->id_pkl}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                      data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
                      <form action="/dashboard/d/{{$p -> id_pkl}}" method="post" class="d-inline">
                        @method('delete')
                        @csrf
      ...
      <!-- panel table pKL -->
        ...
        @can('mhs_dosen')
          <header class="panel-heading">
          <div class="export-fitur">
              <h3 class="panel-title">Daftar Laporan Kuliah Kerja Nyata (KKN)</h3>
              <!-- <button type="button" class="btn btn-primary pdf">Export PDF</button> -->
              <!-- <a href="/dashboard/exportKkn" class="btn btn-primary pdf">Report PDF KKN</a> -->
              <!-- <button type="button" class="btn btn-info print">Print</button> -->
            </div>
          </header>
          @endcan
                ...
                <tr class="gradeA">
                  ...
                  <td class="actions">
                    ...
                    <a href="{{ URL::to('/')}}/storage/{{$d->file_kkn}}" class="btn btn-sm btn-icon btn-pure btn-default on-default download-row"
                      data-toggle="tooltip" data-original-title="download" target="_blank"><i class="icon wb-download" aria-hidden="true"></i></a>
                    <a href="/dashboard/edit/{{$d->id_kkn}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                      data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
                      <form action="/dashboard/{{$d->id_kkn}}" method="post" class="d-inline">
                        @method('delete')
                        @csrf
                        <button class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row" type="submit" data-toggle="tooltip" data-original-title="Remove" onclick="return confirm('Anda Yakin Mau Hapus Data ?')"><i class="icon wb-trash" aria-hidden="true"></i></button>
                      </form>
